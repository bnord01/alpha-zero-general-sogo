...
class HumanConnect4Player():
    ...
    def play(self, board):
        valid_moves = self.game.valid_actions(board, 1)
...
class OneStepLookaheadConnect4Player():
    ...
    def play(self, board):
        valid_moves = self.game.valid_actions(board, self.player_num)
        win_move_set = set()
        fallback_move_set = set()
        stop_loss_move_set = set()
        for move, valid in enumerate(valid_moves):
            if not valid: continue
            if self.player_num == self.game.terminal_value(*self.game.next_state(board, self.player_num, move)):
