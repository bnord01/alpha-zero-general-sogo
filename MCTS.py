        ...
        for a, child in self.children.items():
            if filter == None:
                ...
                if isinstance(filter, list):
                    ...
                    child.print(limit=limit-1, indent=indent+'  ',
                                action=str(a), filter=filter[a])
...
class Play(object):
    ...
    def apply(self, action):
        self.board, self.player = self.game.next_state(
...
class MCTS():
    ...
    def get_action_prob(self, board, player=1, root=None):
        ...
        for _ in range(self.config.num_mcts_sims - root.visit_count):
            node = root
    ...
    def add_exploration_noise(self, node: Node):
        ...
        noise = np.random.gamma(
            self.config.root_dirichlet_alpha, 1, len(actions))
